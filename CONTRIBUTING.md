...
5. Run tests: `npx jest --config '{}' tests/scripts/`
6. Commit with a clear message (pre-commit hook auto-updates CLAUDE.md)
...
### Unit Tests
```bash
npx jest --config '{}' tests/scripts/
```

### End-to-End Evals
```bash
bash tests/evals/run-evals.sh                                      # Readiness evals
bash tests/evals/run-evals.sh --config setup-eval-config.json      # Setup evals
```

The readiness suite runs `/readiness` against 3 fixture projects and grades with `grader.js`. The setup suite runs `/setup` against 2 fixtures and grades with `setup-grader.js`.
...
- CLAUDE.md auto-updates on commit (AUTO markers), but verify the result looks correct
